...
#     p, n_p, q, n_q = _oversample_chords(mesh, num_samples, sampling_method='random')
#     p, n_p, q, n_q = _prune_chords(p, n_p, q, n_q)

#     return p[:num_samples], n_p[:num_samples], q[:num_samples], n_q[:num_samples]
...
def plane2_chord_representation(mesh, num_samples=100, sampling_method='uniform', cluster=False):
    p, n_p, q, n_q = _sample_chords(mesh, num_samples, sampling_method)

    return plane2_aux(p, n_p, q, n_q)


def plane2_aux(p, n_p, q, n_q):
    '''
    Input
    m - (n,) signed distance between p and q
    n_p - (n, 3) normal at point p
    n_q - (n, 3) normal at point q
    '''
